...
- Using the information below, or from the public dataset itself:
	- Create a query that merges two tables together, assuming one table contains updated data on players that you want to store in the existing table.
		- 1.  **Check if the player exists**  in the  `players`  table.
		- 2.  **If the player exists**, update their points and team.
		- 3.  **If the player does not exist**, insert a new record.
	- Create a query that provides a comprehensive view of teams, their team market, team color, mascot name, count of players, and average points.
